...
namespace SF {
	...
	void	MakeNeHyperplaneList_v(PT_vector_T u, int* neHyperplanes_all, int mneh_all, int* neHyperplanes_v, int* mneh_v, double eps_on_hyperplane);
	...
	bool	MPS__MakeProblem(PT_MPS_row_T* row, int n_row, PT_MPS_column_T* column, int n_col, double* loBound, PT_MPS_vpBound_T* upBounds, int n_vp, PT_MPS_fxVariable_T* fxVariable, int n_fx);
	bool	MPS__ReadBounds(FILE* stream, PT_MPS_column_T* column, int n_col, double* loBound, PT_MPS_vpBound_T* upBound, int* n_vp, PT_MPS_fxVariable_T* fxVariable, int* n_fx);

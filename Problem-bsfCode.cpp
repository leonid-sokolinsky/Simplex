...
void PC_bsf_Init(bool* success) {
	...
	* success = MTX_LoadVector(PD_v, PP_MTX_POSTFIX_V);
	if (*success == false)
		return;

	#ifdef PP_MAX_OBJ_VALUE
	double ObjF_u_cur = ObjF(PD_v);
	if (RelativeError(PP_MAX_OBJ_VALUE, ObjF_u_cur) < PP_EPS_RELATIVE_ERROR) {
		if (BSF_sv_mpiRank == BSF_sv_mpiMaster) {
			cout << "|PP_MAX_OBJ_VALUE-F(v_cur)|/|PP_MAX_OBJ_VALUE| = " << RelativeError(PP_MAX_OBJ_VALUE, ObjF_u_cur) << " < PP_EPS_ZERO = " << PP_EPS_RELATIVE_ERROR << endl;
	...
	List_equations(PD_isEquation, PD_eqHyperplanes, &PD_meq);
	MakeEquationsBasis(PD_basis_v, &PD_meq_basis, PP_EPS_ZERO);
	...
	PD_mne_v = 0;
	List_neHyperplanes_v(PD_v, PD_neHyperplanes, PD_mne, PD_neHyperplanes_v, &PD_mne_v, PP_EPS_ON_HYPERPLANE);
	assert(0 < PD_mne_v && PD_mne_v <= PP_MM);

	int rank;
	Matrix_Rank(PD_neHyperplanes_v, PD_mne_v, &rank, PP_EPS_ZERO);
	if (rank < PD_subspaceDim) {
		if (BSF_sv_mpiRank == BSF_sv_mpiMaster)
			cout << "PC_bsf_Init error: v is NOT vertex with precision of PP_EPS_ON_HYPERPLANE = "
...
void PC_bsf_MapF(PT_bsf_mapElem_T* mapElem, PT_bsf_reduceElem_T* reduceElem, int* mapSuccess) {
	...
	if (NegativeNotFoundIn_u(PP_EPS_ZERO)) {
		reduceElem->negativeNotFoundIn_u = true;
		*mapSuccess = true;
		return;
	}

	reduceElem->negativeNotFoundIn_u = false;
	...
	for (int i = 0; i < PD_m; i++) {
		...
		if (PD_u[i] < -PP_EPS_ZERO) {
			...
			if (entryNumber == mapElem->entryNumber) {
				...
				assert(PointBelongsToPolytope(reduceElem->v_nex, PP_EPS_ON_HYPERPLANE));
...
void PC_bsf_ParametersOutput(PT_bsf_parameter_T parameter) {
	...
	#ifdef PP_LOAD_BASIS
	cout << "Load basis: Yes" << endl;
	#else 
	cout << "Load basis: No" << endl;
	#endif // PP_LOAD_BASIS
...
void PC_bsf_ProblemOutput(PT_bsf_reduceElem_T* reduceResult, int reduceCounter, PT_bsf_parameter_T parameter, double t) {
	...
	cout << "// Computed objective value: " << setprecision(24) << PD_objF_v << endl;
	#ifdef PP_MAX_OBJ_VALUE
	cout << "// Maximal objective value:  " << PP_MAX_OBJ_VALUE << endl;
	cout << "// Relative error = " << setprecision(3) << PD_relativeError << setprecision(PP_SETW / 2) << endl;
	#endif // PP_MAX_OBJ_VALUE
	cout << "// Distance to polytope: " << Distance_PointToPolytope(PD_v) << endl;
	cout << "=================================================" << endl;

	#ifdef PP_SAVE_RESULT
	if (MTX_SaveVector(PD_v, PP_MTX_POSTFIX_SO))
...
void PC_bsf_ProcessResults(PT_bsf_reduceElem_T* reduceResult, int reduceCounter, PT_bsf_parameter_T* parameter, int* nextJob, bool* exit) {
	...
	if (!PointIsVertex(reduceResult->v_nex, PP_EPS_INVERSE, PP_EPS_ON_HYPERPLANE)) {
		cout << "PC_bsf_ProcessResults error: reduceResult->v_nex is NOT vertex with precision of PP_EPS_ON_HYPERPLANE = "
			<< PP_EPS_ON_HYPERPLANE << " and PP_EPS_INVERSE = " << PP_EPS_INVERSE << endl;
		*exit = true;
		return;
	}
	#endif // _DEBUG

	PD_objF_v = ObjF(reduceResult->v_nex);
	#ifdef PP_MAX_OBJ_VALUE
	PD_relativeError = RelativeError(PP_MAX_OBJ_VALUE, PD_objF_v);
	#endif // PP_MAX_OBJ_VALUE

	if (reduceResult->negativeNotFoundIn_u) {
		#ifdef PP_MAX_OBJ_VALUE
		if (PD_relativeError < PP_EPS_RELATIVE_ERROR) {
	...
	if (reduceCounter == 0) {
		cout << "Try detect #define PP_RND_SEED" << endl;
		*exit = true;
		return;
	}
	...
	char buf[10];
	sprintf(buf, "%d", (int)fabs(PD_objF_v * PP_SCALE_FACTOR));
	string postfix = "_v(" + string(buf) + ").mtx";
	if (MTX_SaveVector(PD_v, postfix))
...
void PC_bsf_ReduceF(PT_bsf_reduceElem_T* x, PT_bsf_reduceElem_T* y, PT_bsf_reduceElem_T* z) { // z = x o y
	if (x->negativeNotFoundIn_u) {
		z->negativeNotFoundIn_u = true;
		assert(y->negativeNotFoundIn_u);
		return;
	}
...
namespace SF {
	...
	static inline void List_neHyperplanes_v(PT_vector_T v, int* neHyperplanes, int mneh, int* neHyperplanes_v, int* mneh_v, double eps_on_hyperplane) {
		// List of hyperplanes that are not equations and include point v.
	...
	static void Matrix_Basis(int* list_i, int* mi, double eps_inverse) {
		int rank;
		int meq_total = *mi;
		int firstForProbe = 0;
		int probeRank;

		Matrix_Rank(list_i, *mi, &rank, eps_inverse);

		for (int check_count = 0; check_count < meq_total; check_count++) { // always check the last
			Matrix_Rank(list_i, (*mi) - 1, &probeRank, eps_inverse);
			if (probeRank == rank)
				(*mi)--;
			else {
	...
	static void Matrix_Rank(int* list_i, int mi, int* rank, double eps_inverse) {
		double factor;
	...
	static inline bool PointBelongsToHalfspace_i(PT_vector_T x, int i, double eps_on_hyperplane) {
		...
		if (a_DoT_x_MinuS_b < eps_on_hyperplane)
			return true;
	...
	static inline bool Vector_i_Equal(PT_vector_i_T x, PT_vector_i_T y) { // x = y
		for (int j = 0; j < _n; j++)
			if (x[j] != y[j])
				return false;
		return true;
...
namespace PF {
	...
	static inline void AddToBasis(int i, int* basis, int m_basis, bool* success, double eps_inverse) {
		basis[m_basis] = i;
		if (m_basis == 0) {
			*success = true;
			return;
		...
		int rank;
		Matrix_Rank(basis, m_basis + 1, &rank, eps_inverse);
		if (rank == m_basis + 1)
			*success = true;
		else
			*success = false;
	...
	static inline void LoadBasis(PT_vector_i_T basis, bool* success) {
		string postfix = "_i.mtx";
		*success = MTX_LoadVector_i(basis, postfix);
	}

	static inline void Make_y(PT_vector_i_T basis_v, int i_star, PT_vector_T y) {
		bool ok = false;
		for (int j = PD_meq_basis; j < PD_n; j++)
			if (basis_v[j] == i_star) {
	...
	static inline void MakeBasis_v(int* neHyperplanes_v, int mne_v, int meq_basis, int* basis_v, double eps_zero) {
		int m_v = meq_basis;

		for (int i = 0; i < mne_v; i++) {
			bool success;
			AddToBasis(neHyperplanes_v[i], basis_v, m_v, &success, eps_zero);
			if (success) {
				m_v++;
	...
	static inline void MakeEquationsBasis(int* eqBasis, int* m_eqBasis, double eps_zero) {
		...
		*m_eqBasis = 0;
		...
		eqBasis[*m_eqBasis] = PD_eqHyperplanes[0];
		(*m_eqBasis)++;

		bool success;
		for (int i = 1; i < PD_meq; i++) {
			AddToBasis(PD_eqHyperplanes[i], eqBasis, *m_eqBasis, &success, eps_zero);
			if (success)
				(*m_eqBasis)++;
		}
	}

	static inline void MakeRndBasis_v(int* neHyperplanes_v, int mne_v, int meq_basis, int* basis_v, double eps_zero) {
		int m_v = meq_basis;
		PT_bitscale_T bitscale;

		Bitscale_False(bitscale, _m);

		while (m_v < _n) {
			int rnd_i;
			do {
				rnd_i = rand() % mne_v;
			} while (bitscale[rnd_i]);
			bitscale[rnd_i] = true;
			bool success;
			AddToBasis(neHyperplanes_v[rnd_i], basis_v, m_v, &success, eps_zero);
			if (success) {
				m_v++;
				#ifdef PP_BASIS_GAUGE
	...
	static inline bool NegativeNotFoundIn_u(double eps_zero) {
		for (int i = 0; i < _m; i++) {
			if (PD_isEquation[i])
				continue;
			if (PD_u[i] < -eps_zero)
	...
	static inline bool PointIsVertex(PT_vector_T v, double eps_zero, double eps_on_hyperplane) {
		int hyperplanes_v[PP_MM];
		int mh_v = PD_meq_basis;
		int rank;

		for (int i = 0; i < PD_meq_basis; i++)
			hyperplanes_v[i] = PD_eqHyperplanes[i];

		List_neHyperplanes_v(v, PD_neHyperplanes, PD_mne, hyperplanes_v, &mh_v, eps_on_hyperplane);

		Matrix_Rank(hyperplanes_v, mh_v, &rank, eps_zero);
		if (rank < PD_n)
			return false;
		else
			return true;
	...
	static inline void Pre_Make_A0I(double eps_inverse, bool* success) {
		double factor;
		...
		*success = true;
		...
		for (int j = 0; j < _n; j++) {
			...
			if (fabs(_D[j][j]) <= eps_inverse) { // Search for a non-zero below
				int i_ne_0 = -1;
	...
	static inline void PreparationForIteration(PT_vector_i_T basis_v) {
		...
		#ifdef _DEBUG
		int rank;
		Matrix_Rank(basis_v, _n, &rank, PP_EPS_ZERO);
		assert(rank == _n); // Try to decrease PP_EPS_ZERO
		#endif // _DEBUG

		Pre_Make_A0(basis_v);

		bool success;
		Pre_Make_A0I(PP_EPS_INVERSE, &success);
		assert(success);
